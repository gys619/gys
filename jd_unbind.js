...
async function unsubscribeCards() {
  ...
  push_len = parseInt($.pushcardList.length/2/10)
  push_lenb = $.pushcardList.length/2%10

  if (push_len == 0){
    let tg_text = ''
    for (card in $.pushcardList){
      tg_text = tg_text + $.pushcardList[card] + '\n'
    }
    if ($.pushcardList.length > 0){
      await notify.sendNotify(`京东会员卡注消链接`, `${$.nickName}\n${tg_text}`);
    } else {
      await notify.sendNotify(`京东会员卡注消链接`, `${$.nickName}\n卡包🈚️卡`);
    }
  } else if (push_lenb == 0){
    for (step = 0; step < push_len; step++) {
      let tg_text = ''
      for (card in $.pushcardList[step*20,step*20+19]){
        tg_text = tg_text + $.pushcardList[card] + '\n'
      }
      await notify.sendNotify(`京东会员卡注消链接`, `${$.nickName}\n${tg_text}`);
    }
    ...
    let tg_text = ''
    for (card in $.pushcardList[push_len*20,push_len*20+push_lenb*2-1]){
      tg_text = tg_text + $.pushcardList[card] + '\n'
    }
    await notify.sendNotify(`京东会员卡注消链接`, `${$.nickName}\n${tg_text}`);
  }  
